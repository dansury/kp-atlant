...
## Куда ведёт нажатие (FR-073)

Push, который открывает просто «панель», заставляет искать письмо руками — ровно то,
от чего модуль должен избавить. Цель уведомления теперь хранится, а не угадывается:

- `notifications.url` — адрес, на который ведёт и push, и кнопка «Открыть» в списке
  уведомлений. `Notifier::notify()` принимает его последним аргументом; когда вызывающий
  молчит, адрес выводится из `ref_type` (`request` → `#mail/request/<id>`,
  `mail` → `#mail/msg/<id>`, `proposal`, `counterparty`).
- Синхронизация почты знает точное письмо и передаёт `#mail/msg/<id>`: «пришло новое
  письмо» открывает именно его, а созданный из него запрос — на одну ссылку внутрь.
- `tag` push-сообщения теперь свой у каждой цели (`kind:<хеш адреса>`), а не один на тип:
  иначе второе письмо молча заменяло первое, и нажатие открывало не то письмо.
- `sw.js` при нажатии наводит уже открытую вкладку на адрес: сначала `client.navigate()`,
  а если воркер этой вкладкой не управляет — `postMessage`, который клиент отрабатывает
  сменой хеша. Закрытую панель открывает `clients.openWindow()`.

## Включение из «Уведомлений» (FR-074)

Карточка push (включить на этом устройстве, «Проверить», типы, диагностика) теперь
стоит и на странице «Уведомления» — там, где менеджер оказывается, когда уведомления
не приходят, — а не только в «Настройки → Это устройство». Это один и тот же
`renderPushCard()`, а не вторая реализация.
...
notifications.url                              -- куда ведёт нажатие (схема v14)
```

Версия схемы 9, адрес уведомления добавлен в v14.
