...
ok('доля первой позиции верна (1000 из 4000 → +75)', str_contains($html, '1 075,00 руб.'), $html === '' ? '' : 'см. документ');
ok('доля второй позиции — остаток копеек (+225)', str_contains($html, '3 225,00 руб.'));
ok('«Итого» — сумма товаров и доставки, как и раньше', str_contains($html, 'Итого: 4 300,00 руб.'));
...
ok('и прежние цены позиций без надбавки', str_contains($htmlLine, '1 000,00 руб.') && str_contains($htmlLine, '3 000,00 руб.'));
ok('«Итого» то же самое', str_contains($htmlLine, 'Итого: 4 300,00 руб.'));
