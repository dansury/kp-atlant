...
try {
    ...
    ok('таблица соответствия в документе', str_contains($html, 'Таблица соответствия запросу'));
    ok('в ней — что просили', str_contains($html, 'Кираса-5'));
    ok('и что предлагаем', str_contains($html, 'Страж-5'));
    ok('замена названа заменой', str_contains($html, 'аналог'));
    ok('перечислены совпавшие требования', substr_count($html, 'соответствует') >= 2,
