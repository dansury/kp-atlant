...
final class Settings {
    ...
    public const SPEC = [
        ...
        // Голосовой ввод (issue #98): модель и язык SpeechKit — отдельно от YandexGPT
        'SPEECH_MODEL'          => ['llm', 'Модель Yandex SpeechKit (голосовой ввод)', 'select:general=general — основная,general:rc=general:rc — новая версия (кандидат),general:deprecated=general:deprecated — прежняя версия', false, 'general', 'Модель распознавания речи SpeechKit STT v1 (параметр topic). general — стабильная, general:rc — более новая, раньше попадает в релиз, general:deprecated — предыдущая, если новая распознаёт хуже. Ключ и каталог — те же, что у YandexGPT'],
        'SPEECH_LANG'           => ['llm', 'Язык голосового ввода', 'select:ru-RU=русский,en-US=английский,kk-KZ=казахский,uz-UZ=узбекский,tr-TR=турецкий,de-DE=немецкий,auto=определять сам', false, 'ru-RU', 'На каком языке диктуют письма. «Определять сам» — SpeechKit угадывает язык, но хуже распознаёт короткие фразы'],
        'SPEECH_PROFANITY'      => ['llm', 'Фильтр ненормативной лексики', 'bool', false, 0, 'SpeechKit заменит бранные слова звёздочками'],
        ...
        'MATCH_AUTO_LLM'       => ['match', 'Нейросеть сама уточняет подбор', 'bool', false, 1, 'Каталог не справился — ничего не нашёл, нашёл несколько равных или нашёл слабо — и нейросеть выбирает позицию сама, одним запросом на письмо. Выбирает только среди того, что нашёл каталог: названия, цены и остатки — из каталога. Без ключа нейросети подбор остаётся каталожным'],
        ...
        'MAIL_SYNC_SEEN'     => ['mail', 'Прочитанное в почте — прочитано и здесь', 'bool', false, 1, 'Письмо, открытое в веб-почте или на телефоне, перестаёт числиться непрочитанным в сервисе: при каждом заборе почты сервис спрашивает у ящика отметку «прочитано» у своих непрочитанных писем. Сам сервис отметок в ящике не ставит'],
