...
PROMPT,
            ...
            'match_pick' => [
                'Подбор позиции нейросетью',
                'Каталог не справился сам — ничего не нашёл, нашёл несколько равных или нашёл слабо (модуль 065). Нейросеть выбирает позицию среди кандидатов, которые уже проверил код, одним запросом на всё письмо. Отвечает строго JSON; названия, цены и остатки берутся из каталога. {{knowledge}} — номенклатура из вики.',
                ['knowledge'],
                <<<'PROMPT'
Ты сопоставляешь строки письма клиента с каталогом Atlant Armour (бронежилеты, шлемы, плиты,
подсумки, тактическая одежда, рюкзаки, тактическая медицина, оптика).

На вход — JSON: {"lines":[{"line":N,"asked":"строка клиента","text":"фрагмент письма",
"size":"размер из письма или null","candidates":[{"id":"...","name":"...","characteristics":"...",
"sizes":"размеры и цвета модификаций","in_stock":true,"description":"..."}]}]}
Кандидатов нашла и проверила система. Твоя работа — для каждой строки выбрать ОДИН кандидат,
который и есть то, что просит клиент, или сказать, что такого нет.

Верни JSON и ничего кроме него:
{"lines":[{"line":N,"pick":"<id кандидата или null>","reason":"<одна фраза по-русски: почему это он>"}]}

Правила:
1. "pick" — только id из candidates этой строки. Выдумать id нельзя.
2. Выбирай ТОТ ЖЕ товар, а не похожий: модель, серия, номер, класс защиты и материал в названии
   должны совпасть с тем, что написал клиент («Атом» ≠ «Атом-2», «Бр3» ≠ «Бр4»). Торговые синонимы —
   одно и то же: броник = бронежилет, каска = шлем, ифак = аптечка, плейт = плитоноска.
3. Размер и цвет не выбирай: их поставит система по "size" и по словам клиента. Не отбрасывай
   кандидата из-за размера или цвета.
4. Подходят несколько — бери тот, чьё название ближе к словам клиента; при равенстве — тот, что in_stock.
5. Ни один кандидат не тот товар — "pick": null. Это нормальный ответ: чужой товар в КП хуже, чем «не нашли».
6. Не называй цены и остатки. "reason" — до 120 знаков, без извинений и без вопросов.

{{knowledge}}
Названия наших моделей из базы знаний выше помогают узнать товар, который клиент назвал по-своему.
PROMPT,
            ],
